...
Optionally, since GitHub's OpenID Connect implementation is not strictly to spec, you may want to create a custom [reconcile lambda](/docs/extend/code/lambdas/reconcile/openid-connect-response-reconcile) function that will enable FusionAuth to get more information about the logged-in user than GitHub normally provides.
...
You can plug in both [Amazon Cognito](/docs/lifecycle/authenticate-users/identity-providers/enterprise/cognito) and [Microsoft Entra ID](/docs/lifecycle/authenticate-users/identity-providers/enterprise/microsoft-entra-id-oidc) to FusionAuth as OpenID Connect providers using the same [generic OpenID Connect workflow](/docs/lifecycle/authenticate-users/identity-providers/overview-oidc) shown in the GitHub section above.
