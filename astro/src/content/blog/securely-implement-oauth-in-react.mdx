...
In addition to React, we'll use a Node.js backend. One major benefit of using a backend server is that we can safely store and use a Client Secret, which is the most secure way to integrate OAuth 2.0 Authorization Code Flow with our React app.

<Aside type="tip" nodark="true">This blog post has been [superseded by a newer React/OAuth tutorial](/blog/how-to-authenticate-your-react-app).</Aside>
...
Select <Breadcrumb>Manage</Breadcrumb> and go to the <Breadcrumb>Registrations</Breadcrumb> tab. Click on <Breadcrumb>Add Registration</Breadcrumb>, pick your new application, and then <Breadcrumb>Save</Breadcrumb> to register yourself.
...
The OAuth and FusionAuth configuration above will not match your application. I copied it out of my FusionAuth admin panel. This is the only code in the whole article you can't just copy/paste, seriously. The best place to find your values is this <Breadcrumb>View</Breadcrumb> button in the Applications page:
...
One thing we don't cover here is creating an API key in FusionAuth and copying it over to our `config.js` file. This process is simple and you can read our [API Authentication](/docs/apis/authentication) documentation to learn more.
...
First thing's first: we need a <Breadcrumb>log in</Breadcrumb> button in the React client:
...
The last thing to do is add the `UserData` component, which will be a `<textarea>`. We can lock it using the <InlineField>readonly</InlineField> attribute when there's no user logged in, and we can apply our `handleTextInput` method to the `onChange` event.
...
Once everything is hooked up, you can test it out by typing something into the `<textarea>` in the React app and then verifying that the data is updated in FusionAuth. The user data is displayed in FusionAuth on the Manage page for the user under the <Breadcrumb>User data</Breadcrumb> tab:
...
- You'll probably want to let users [register from the app itself](/docs/apis/registrations#create-a-user-and-registration-combined), instead of manually adding them via the admin panel.
- Maybe you want to [change how the FusionAuth login form looks](/docs/apis/themes).
- For extra security, you can [enable and send two-factor codes](/docs/apis/two-factor).
- You can even let users log in via a third-party, like [Facebook](/docs/apis/identity-providers/facebook) or [Google](/docs/apis/identity-providers/google).
...
Check out the [FusionAuth APIs](/docs/apis/) for even more options.
