...
htmlTitle: "What Is GNAP? A New Authorization Protocol Beyond OAuth2"
description: "GNAP (Grant Negotiation and Authorization Protocol) is a next-gen auth standard under development. Learn how it differs from OAuth2 and why it matters."
image: /img/blogs/gnap-oauth2-next-gen/gnap-the-next-generation-of-oauth-header-image.png
authors: Dan Moore
categories: Education
tags: oauth, gnap, standards
...
![The pyramid of OAuth2 specifications.](/img/blogs/gnap-oauth2-next-gen/oauth2-spec-pyramid.svg)

GNAP isn't alone in trying to fix some of these issues. [OAuth 2.1](/articles/oauth/differences-between-oauth-2-oauth-2-1) is addressing some of them as well. From my research, including reading the draft spec and reviewing discussion on the mailing list, GNAP aims to address these problems in a more cohesive manner. 

> _Curious how FusionAuth stays ahead of evolving standards like GNAP? [Schedule a personalized demo](/contact) and see how our platform future-proofs your authentication stack._
...
In some cases, the <InlineField>key</InlineField> attribute has the value `true`; in this case the token is bound to the key of the client previously presented to the authorization server.
...
First, if OAuth2 works for you, keep using it. Of course, make sure you use it properly, with all the security improvements documented in [OAuth 2.1](/articles/oauth/differences-between-oauth-2-oauth-2-1). OAuth2 is not going anywhere.
