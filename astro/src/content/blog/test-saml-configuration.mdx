...
You will need an instance of FusionAuth up and running. You can do so using [Docker](/docs/get-started/download-and-install/docker), using [Homebrew](/docs/get-started/download-and-install/homebrew), or [a number of other methods](/download).

### 1. Create A New Application Named IAMShowcase Example

Log into FusionAuth and select <Breadcrumb>Applications</Breadcrumb>. Give the application a <InlineField>name</InlineField> such as <InlineFieldValue>IAMShowcase Example</InlineFieldValue>. If you have more than one tenant, select the default <InlineField>Tenant</InlineField>. Then click the blue disk to save the configuration.
...
Select <Breadcrumb>Applications</Breadcrumb> and <Breadcrumb>Edit</Breadcrumb> the IAMShowcase Example application in FusionAuth. Select the <Breadcrumb>SAML</Breadcrumb> tab of the application and configure it with the following values.

<Breadcrumb>Enabled</Breadcrumb> `true`

<InlineField>Issuer</InlineField> <InlineFieldValue>IAMShowcase</InlineFieldValue>
...
<InlineField>Authorized Redirect URLs</InlineField> <InlineFieldValue>https://sptest.iamshowcase.com/acs</InlineFieldValue>
...
<md:AssertionConsumerService Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" Location="https://sptest.iamshowcase.com/acs" index="0" isDefault="true"/>
```
<InlineField>Signing key</InlineField> <InlineFieldValue>Auto generate a new key on save...</InlineFieldValue>
...
Select <Breadcrumb>Users</Breadcrumb>. Choose a user you want to test the SAML log in process with. Select <Breadcrumb>Manage</Breadcrumb> and then click the <Breadcrumb>Add registration</Breadcrumb> button for the IAMShowcase Example application. Click the save icon and you have registered the user for the application.
...
Click on <Breadcrumb>Applications</Breadcrumb>. Select <Breadcrumb>View</Breadcrumb> for `IAMShowcase Example`. Scroll down to the <Breadcrumb>SAML v2 integration details</Breadcrumb> section. Copy the <Breadcrumb>Metadata URL</Breadcrumb> and paste that into a new browser window to get the XML metadata.
...
Paste the XML into the [Upload metadata](https://sptest.iamshowcase.com/instructions#start) box on the IAMShowcase site. Click <Breadcrumb>Submit XML</Breadcrumb>. Copy the URL that pops up to your clipboard.
...
```mermaid title="SAML login flow."
sequenceDiagram

participant User
participant SP as IAMShowcase (SP)
participant IdP as FusionAuth (IdP)

User->>SP: Requests access to resource
SP->>SP: Generate SAML AuthnRequest
SP->>User: Redirect user to IdP with SAML AuthnRequest
User->>IdP: Send SAML AuthnRequest to FusionAuth
IdP->>IdP: Authenticate user (e.g., login)
IdP->>User: Respond with SAML Assertion
User->>SP: Send SAML Assertion to IAMShowcase
SP->>SP: Validate SAML Assertion
SP->>User: Grant access to resouce
```
