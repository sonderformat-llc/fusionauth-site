...
seoTitle: "How to Migrate from Azure AD B2C | Azure B2C Migration Guide"
description: Planning to migrate from Azure AD B2C? This guide explains how to export users, handle password hashes, and transition to a new identity provider securely.
authors: Dan Moore
...
_[Schedule a demo](/contact) to learn how FusionAuth can support a smooth and secure transition from Azure AD B2C._
...
user ->> ad  : Request login page
ad -->> user  : Receive login page
user ->> ad  : Post credentials
ad ->> ad : Verify credentials
ad -->> user  : Display results
```
...
user ->> sys  : Request login page
sys -->> user  : Receive login page
user ->> sys  : Post credentials
sys ->> sys : Verify credentials
sys -->> user  : Display results
```
...
sys -->> user : Receive login page
user ->> sys : Post credentials
sys ->> ad : Post credentials
ad ->> ad  : Verify credentials
ad -->> sys : Return results of authentication, including user attributes
sys -->> user : Display results. If authentication successful, store user in New System.
```
...
If you are looking for step by step instructions on how to move to FusionAuth from Azure AD B2C, please check out our [Azure AD B2C migration guide](/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id).
