...
* [Configure OpenID Connect with Microsoft Entra ID](/docs/lifecycle/authenticate-users/identity-providers/enterprise/microsoft-entra-id-oidc)
...
<APIBlock>
  ...
  <APIField name="Discover endpoints">
    ...
    For example, if `https://login.piedpiper.com` is specified as the issuer, the well-known OpenID Connect URL `https://login.piedpiper.com/.well-known/openid-configuration` will be queried to discover the well-known endpoints.
...
* The FusionAuth redirect URL, which handles the response from the OpenID Connect identity provider. This will always have the path `/oauth2/callback`. If your FusionAuth server is running at `login.piedpiper.com`, the FusionAuth redirect URL will be `https://login.piedpiper.com/oauth2/callback`.

Here is the request flow through an OpenID Connect login where the application is Pied Piper, the authorization server is FusionAuth, and the remote IdP is Microsoft Entra ID.
...
sequenceDiagram
  ...
  f ->> b : Send Login Page
  b ->> f : Clicks on 'Login With Microsoft Entra ID'
  f ->> b : Redirects to Microsoft Entra ID
