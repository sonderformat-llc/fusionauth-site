...
To create an Identity Provider navigate to <Breadcrumb>Settings -> Identity Providers</Breadcrumb> and click <Breadcrumb>Add provider</Breadcrumb> and select `External JWT` from the dialog. This will take you to the Add panel, and you will need to fill out the required fields.

![Add External JWT](/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-add.png)
...
<APIBlock>
  ...
  <APIField name="Reconcile lambda" optional since="1.17.0">
    A lambda may be utilized to map custom claims from the JWT. This may be used in addition to, or in place of the claim mapping on the <Breadcrumb>JWT</Breadcrumb> tab.
...
![Add External JWT](/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-jwt-add.png)
...
A domain may only be managed by a single identity provider in the context of a given tenant. More than one domain may be added per identity provider. Learn more about [managed domains](/docs/lifecycle/authenticate-users/identity-providers/#managed-domains).

![Add External JWT](/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-domains-add.png)
...
All configured claim mappings will be visible here. Use the <Breadcrumb>Add Claim</Breadcrumb> button to add additional claim mappings. You may optionally perform all of the claim mapping using a Reconcile lambda function.

## Add Claim Mapping Dialog

If you click on the <Breadcrumb>Add Claim</Breadcrumb> button on this page you will be presented with the following dialog.

![Add Claim Mapping](/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-add-claim-dialog.png)
...
<APIBlock>
  ...
  <APIField name="FusionAuth claim" required>
    ...
    For example, if you select <InlineFieldValue>company</InlineFieldValue> as the incoming claim, and specify `RegistrationData`, the claim will be stored in the registration data for the corresponding application.
...
![Add External JWT](/img/docs/lifecycle/authenticate-users/identity-providers/external-jwt/identity-provider-external-jwt-oauth-add.png)
