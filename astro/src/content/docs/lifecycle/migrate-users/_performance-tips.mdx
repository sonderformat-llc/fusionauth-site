...
* Run with the <InlineField>validateDBConstraints</InlineField> set to <InlineFieldValue>false</InlineFieldValue>.
...
* If you only provide a <InlineField>password</InlineField> field, then FusionAuth will assume the password is in plaintext and hash it for you. Hashing a password negatively affects load time, performance and throughput. If you provide the `salt`, `password`, `encryptionScheme` and `factor` values when importing, then FusionAuth assumes the value in the <InlineField>password</InlineField> field is a hashed password, and it will not be hashed.
