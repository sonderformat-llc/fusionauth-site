---
title: Migrate From Microsoft Entra External ID
description: Learn how to migrate your users to FusionAuth from Microsoft Entra External ID.
---
...
import OneTenantOrMany from 'src/content/docs/lifecycle/migrate-users/provider-specific/_one-tenant-or-many.mdx';
...
export const migration_source_name = 'Microsoft Entra External ID';
...
This document will help you migrate your users off of Microsoft Entra External ID and onto FusionAuth.

There are a number of different ways applications can be integrated with Microsoft Entra External ID, and it would be difficult to cover them all. This guide focuses on migrating user data, including profile data and passwords. However, Microsoft Entra External ID does not allow for password hash export. This is a common security restriction for managed Identity Providers to prevent the exposure of sensitive credential data. Therefore, you must perform a slow migration if you don't want to force users to reset their passwords.

Alternatively, you can do a bulk migration and force everyone to reset their passwords. This option is discussed below, but the primary focus of this guide is enabling you to migrate your users from Microsoft Entra External ID without requiring any password resets.

## Prerequisites

This guide assumes intermediate level familiarity with Microsoft Azure, in particular Microsoft Entra External ID.
...
To preserve your users' passwords, you need to perform a slow migration. Users log in to FusionAuth with their Microsoft Entra External ID credentials and FusionAuth transparently migrates their data. Slow migrations in FusionAuth use Connectors, a paid feature.
...
<OneTenantOrMany migration_source_name={migration_source_name} external_tenant_name="Tenant" external_tenant_name_plural="Tenants" />

### Identity Providers

In Microsoft Entra External ID, external identity providers are managed using [Identity Providers](https://learn.microsoft.com/en-us/entra/external-id/customers/concept-identity-providers). With FusionAuth, these are also called [Identity Providers](/docs/lifecycle/authenticate-users/identity-providers/).
...
<Aside type="note">
Importing Microsoft Entra External ID users stored in an external identity provider is more straightforward because you can use the Import API; you don't have to perform a slow migration. Many of the steps in [Bulk Migration](#bulk-migration) will apply.
</Aside>

To retrieve the user information, use the approach documented in the [Export User Data From Microsoft Entra External ID](#export-user-data-from-microsoft-entra-external-id) section.
...
* [User Flows and Custom Policies](https://learn.microsoft.com/en-us/entra/external-id/customers/concept-user-flow-custom-policy-overview) are ways for you to customize authentication or authorization workflows with Microsoft Entra External ID. FusionAuth has a similar concept called [Lambdas](/docs/extend/code/lambdas/). FusionAuth also has [webhooks](/docs/extend/events-and-webhooks/) fired at certain points in the user lifecycle; in certain configurations, they can also stop a particular authentication flow.
* Microsoft Entra External ID does not have a standardized roles-and-permissions concept. Roles and permissions are usually handled with custom attributes or secondary data stores. FusionAuth has [roles](/docs/get-started/core-concepts/roles) that are configured on an application by application basis and made available in a token after a successful authentication.
* Microsoft Entra External ID has the concept of [User Flow Page Layouts](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-customize-ui-user-flow) where users can log in or register. FusionAuth has [hosted login pages](/docs/get-started/core-concepts/integration-points#hosted-login-pages) which offer a superset of the functionality of Microsoft Entra External ID User Flow. They are [customized via themes](/docs/customize/look-and-feel/).
* In Microsoft Entra External ID, users log into [App Registrations](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-register-ciam-app). In FusionAuth, [Applications](/docs/get-started/core-concepts/applications) are a similar construct, but users are associated with them through [Registrations](/docs/get-started/core-concepts/registrations). You can migrate both your Client Id and Client Secret from Microsoft Entra External ID to FusionAuth.
* Microsoft Entra External ID sends emails on your behalf, such as forgot password notifications. FusionAuth can do so too; [the templates are customizable](/docs/customize/email-and-messages/).
* Microsoft Entra External ID offers the Client Credentials grant. FusionAuth offers a constrained version of this using [Entity Management](/docs/get-started/core-concepts/entity-management); this is available in all paid plans.
* Refresh tokens allow JWTs to be refreshed without a user logging in. These can be migrated from Microsoft Entra External ID using the [Import Refresh Tokens API](/docs/apis/users#import-refresh-tokens).
* Microsoft Entra External ID allows for custom attributes, but they must be configured at the Directory level. In FusionAuth, as mentioned above, custom user attributes are stored on the `user.data` field and are dynamic, searchable and unlimited in size. Any valid JSON value may be stored in this field.
* Microsoft Entra External ID supports MFA. FusionAuth also [supports MFA](/docs/lifecycle/authenticate-users/multi-factor-authentication), which may be enabled for a tenant and configured for a user at any time.
...
Microsoft Entra External ID, in contrast, gives users access to all Microsoft Entra External ID applications in a Tenant by default.
</Aside>
...
In addition to the different names for common functionality outlined above in [Other Entities](#other-entities), there are some fundamental differences between FusionAuth and Microsoft Entra External ID. If your application relies on Microsoft Entra External ID specific functionality, please review this section carefully.

* Microsoft Entra External ID has [a pricing model](https://azure.microsoft.com/en-us/pricing/details/active-directory/external-identities/#pricing) that applies to monthly active users (MAUs). FusionAuth has no user limit. Instead, you are limited by the resources provided to a FusionAuth instance, such as memory, CPU and database capacity.

Once you've planned the migration of other entities, the next step is to set up FusionAuth to connect to Microsoft Entra External ID to import users during login.
...
### Set Up Microsoft Entra External ID

There are three main components you need to set up to enable this migration.

The first is an App Registration, with an [ROPC flow](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-ropc-flow).

Secondly, you will need to set up a [Microsoft Graph application](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-use-microsoft-graph-ciam) to retrieve the user's details, in order to import them to FusionAuth.

The third is an [Azure Function](https://learn.microsoft.com/en-us/azure/azure-functions/functions-overview) to accept login requests from FusionAuth over TLS and perform an [ROPC login](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-ropc-flow) request on Microsoft Entra External ID, and return the relevant user data via the Microsoft Entra External ID Graph API app.

<Aside type="caution">
FusionAuth Connectors will send user credentials to Microsoft Entra External ID for authentication, via an Azure Function. Secure both the data in transit and the Azure Function endpoint.

In this guide, all communication is over TLS and the Azure Function requires a unique query string code. You should also review the documents on [Securing Azure Functions](https://learn.microsoft.com/en-us/azure/azure-functions/security-concepts?tabs=v4) to implement any additional security features you may require.

Also note that the Microsoft Entra External ID ROPC endpoint is monitored by rate limiting and identity protection systems. If a dynamic threshold of failed authentications is exceeded, the identity protection system may identify a repeated IP address (i.e. the Azure Function) as an attacker. You may need to deploy Functions to multiple regions, or perhaps self-host the function on a known IP address to mitigate potential issues.
</Aside>

#### Configuring the App Registration in Microsoft Entra External ID

FusionAuth will send login requests to Microsoft Entra External ID to validate the users credentials, via the Azure Function. It will also retrieve the users Microsoft Entra External ID profile from the Microsoft Graph API. It is best practice to create new App Registrations for each of the ROPC flow and Microsoft Graph connection. This guide will walk through the configurations using the Microsoft Entra admin center.

<Aside type="note">
These instructions use the newer [App Registrations](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-register-ciam-app) experience.
</Aside>

Visit the [Microsoft Entra admin center](https://entra.microsoft.com/). Then navigate to <Breadcrumb>Identity -> Applications -> App registrations</Breadcrumb> and [follow these instructions](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-ropc-flow#register-an-application) to set up a new ROPC App Registration and Flow.

At the end, you'll end up with an app client configuration looking similar to this:

![Configuring an ROPC App Registration for migration.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/ropc-app-registration-configuration.png)
...
![Finding the app client id.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/ropc-app-client-id.png)

Now you can set up the App Registration for the Microsoft Graph API access. Follow [this guide](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-use-microsoft-graph-ciam), including the section to create a client secret. Record both the Client Secret and the Application (client) Id as above.

Next, you'll need to set up an Azure Function to receive the login requests from FusionAuth and return the user's profile. The Function will pass the credentials on to Microsoft Entra External ID and, when the authentication succeeds, retrieve the user's profile from Microsoft Graph, then return a FusionAuth friendly JSON object.

#### Configuring the Azure Function

The Azure Function receives the login request from FusionAuth, attempts to log the user in via ROPC, and then returns a FusionAuth user object on success. The Function will call the [Users](https://learn.microsoft.com/en-us/graph/api/user-get) API after successful authentication to get additional user attributes, which will then be transformed into a FusionAuth-compatible format.
...
<RemoteCode url="https://raw.githubusercontent.com/FusionAuth/fusionauth-example-ropc-azure-function/main/RopcProxyFunction/index.js"
            lang="javascript"
            title="Microsoft Entra External ID Connector Function code" />

You will need to modify the `transformToFusionUserObject` function to match the properties of the users that you want to import. This converts the JSON returned from Microsoft Entra External ID into the JSON format FusionAuth requires. You can see samples of both below. To modify this procedure, clone or fork the [repo for the full Function project](https://github.com/FusionAuth/fusionauth-example-ropc-azure-function), then make the changes in your local copy of the project.
...
```json title="Sample User Data Response from Microsoft Entra External ID"
{
...
Azure Functions can be coded within the Azure portal directly, or coded on your local machine and deployed as a package using [Visual Studio Code, with Azure Extensions](https://learn.microsoft.com/en-us/azure/azure-functions/functions-develop-vs-code?tabs=csharp). As the function has dependent modules, you will need to follow these instructions. Clone the [function GitHub repo](https://github.com/FusionAuth/fusionauth-example-ropc-azure-function), and follow the instructions in the link to deploy to your Azure Function.

After deploying the Function, set the following Function environment variables, also known as [Application Settings](https://learn.microsoft.com/en-us/azure/azure-functions/functions-how-to-use-azure-function-app-settings?tabs=portal), to those that you recorded when setting up the ROPC app:

```bash title="Azure Connector Function environment variables to set"
TENANT_NAME:          // The name of your Microsoft Entra External ID Tenant,
...
Now you need to set up a Connector to use the Microsoft Entra External ID API you created. You should set up a different Connector for each Microsoft Entra External ID Tenant.

Log back into the FusionAuth admin UI if needed.
...
* Add a name like `Microsoft Entra External ID migration`.
...
![Configuring a Generic Connector.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/configured-generic-connector.png)

Save the Connector. Next, configure your tenant to use this Connector.

#### Configuring the Tenant

Navigate to your tenant settings: <Breadcrumb>Tenants -> Azure import tenant -> Connectors</Breadcrumb>.

Click the <InlineField>Add policy</InlineField> button to set up a new Connector policy.

![Connector policies for this Tenant.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/connector-policies.png)
...
![Add Connector policy.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/add-connector-policy.png)
...
![Microsoft Entra External ID Connector policy added and in list.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/connector-policy-list-after-adding.png)
...
![Finding the login URL.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/find-login-url.png)
...
![The login page.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/user-login.png)

Enter credentials for a Microsoft Entra External ID user account; it can be the same one you used to test the API with curl and log in. The user will be transparently migrated over to FusionAuth.
...
After enabling enhanced debug messages, try to log in again with the test user. In the admin UI, navigate to <Breadcrumb>System -> Event Log</Breadcrumb> and look for useful messages.
...
That is an important next step but is beyond the scope of this document. Consult the [5 minute setup guide](/docs/get-started/start-here/step-1) for an example of how to do this.
</Aside>

Let's check that the import succeeded in another way: by viewing the user in the admin UI.
...
At this point, you've successfully migrated a user from Microsoft Entra External ID into FusionAuth. Any further changes for this user will occur against the FusionAuth database; this includes profile and password changes.
...
![Deleting a tenant.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/list-of-tenants-delete-highlighted.png)

Confirm your desire to delete the tenant. Depending on how many users exist in that tenant, this may take some time. If it is easier, you may also delete migrated users one at a time using the admin UI.
...
As mentioned above, a bulk migration of Microsoft Entra External ID users requires all imported users to reset their passwords, since Microsoft Entra External ID password hashes are inaccessible. This is typically a poor choice as it negatively affects users. However, if a slow migration won't work because of timing or other reasons, you can move all user information other than passwords from Microsoft Entra External ID to FusionAuth, and then send password reset emails.

The benefits of a bulk migration are:

* You can move your users all at once.
* You no longer have a dependency on Microsoft Entra External ID.
...
* Set up FusionAuth
* Extract all user data from Microsoft Entra External ID
...
The next step is to create an API key. This will be used by the import script. To do so, navigate to <Breadcrumb>Settings -> API Keys</Breadcrumb> in the admin UI.

![Adding an API key](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/add-api-key.png)
...
![Setting API key permissions](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/set-api-key-permissions.png)

### Export User Data From Microsoft Entra External ID

You need to export whatever data you can from Microsoft Entra External ID before you can import it to FusionAuth.

<Aside type="note">
The Microsoft Entra admin center provides for bulk exporting of users. However, at the time of this guide, [user email addresses are not available through the bulk export](https://learn.microsoft.com/en-us/answers/questions/373771/azure-ad-b2c-user-export-missing-mail.html). This guide uses the [Graph API](https://learn.microsoft.com/en-us/graph/use-the-api) to export user data, which does include email addresses.
</Aside>

To export the data via the Graph API, you need to authenticate with the API, and then call the [List method](https://learn.microsoft.com/en-us/graph/api/user-list) to return the data.

To authenticate with Microsoft Entra External ID, you first need to register an application with Microsoft Entra External ID, which will give you an App Id and secret. Follow [this guide](https://learn.microsoft.com/en-us/entra/external-id/customers/how-to-register-ciam-app) to create an application.

We've created a node console tool to authenticate with the Graph API and export the users. You can clone or fork it from [the GitHub repository](https://github.com/FusionAuth/fusionauth-example-azure-ad-bulk-export). Then update the `.env` file with your Microsoft Entra External ID tenant and app ID and secret.
...
Usage: import.rb [options]
    ...
    -u, --users-file USERS_FILE      The exported JSON user data file from Microsoft Entra External ID. Defaults to users.json.
...
FusionAuth Importer : Microsoft Entra External ID
...
The migrated user will have the Microsoft Entra External ID original user identities stored on the `user.data` object. If you have additional user attributes to migrate, review and modify the `map_user` method.
...
Next, log in to the FusionAuth admin UI. Review the user entries to ensure the data was correctly imported.

![List imported users.](/img/docs/lifecycle/migrate-users/provider-specific/microsoft-entra-external-id/list-users.png)
...
Review the `Change Password` template to ensure the messaging is correct. Here is [more information on modifying the template](/docs/customize/email-and-messages/email-templates-replacement-variables#forgot-password).

Retrieve the `loginId` from the Microsoft Entra External ID JSON export files and place it in a single file. The `loginId` will either be the username or the email address. Update the script below to use the API key created in [Set Up FusionAuth For a Bulk Migration](#set-up-fusionauth-for-a-bulk-migration) step.
