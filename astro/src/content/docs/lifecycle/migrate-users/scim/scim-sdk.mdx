...
![Add the SCIM server entity type.](/img/docs/lifecycle/migrate-users/scim/add-scim-server-entity-type.png)
...
![The details page when adding add the SCIM server entity type.](/img/docs/lifecycle/migrate-users/scim/adding-scim-server-entity-type-details.png)
...
![Adding the SCIM client entity type.](/img/docs/lifecycle/migrate-users/scim/add-scim-client-entity-type.png)

Give this entity type an appropriate name such as `SCIM Client`.

![The details of the SCIM client entity type.](/img/docs/lifecycle/migrate-users/scim/adding-scim-client-entity-type-details.png)
...
To add the entities, navigate to <Breadcrumb>Entity Management -> Entities</Breadcrumb> and create a new entity using the <Breadcrumb>+</Breadcrumb> button.

![Adding a SCIM server entity.](/img/docs/lifecycle/migrate-users/scim/add-scim-server-entity.png)
...
![The details of the SCIM server entity.](/img/docs/lifecycle/migrate-users/scim/adding-scim-server-entity-details.png)
...
![Adding a SCIM client entity.](/img/docs/lifecycle/migrate-users/scim/add-scim-client-entity.png)
...
![The details of the SCIM client entity.](/img/docs/lifecycle/migrate-users/scim/adding-scim-client-entity-details.png)
...
![The manage button for the SCIM client.](/img/docs/lifecycle/migrate-users/scim/list-view-highlight-manage-button.png)

Click the <Breadcrumb>Add</Breadcrumb> button to begin the grant adding process.

![The Add Button lets you add grants.](/img/docs/lifecycle/migrate-users/scim/client-grant-add-page.png)
...
![Searching for the correct SCIM server entity.](/img/docs/lifecycle/migrate-users/scim/client-grant-search-page.png)
...
![Granting permissions on the server to the client.](/img/docs/lifecycle/migrate-users/scim/granting-scim-client-permissions-on-scim-server.png)
...
![Finding the Ids for the entities.](/img/docs/lifecycle/migrate-users/scim/getting-ids-for-scim-client-and-server.png)

To find the client secret for the SCIM client entity, view the details screen. You can navigate to the details screen by clicking on the <Breadcrumb>View</Breadcrumb> button (the magnifying glass) from the entity list page.

![Determining the client secret for the SCIM client.](/img/docs/lifecycle/migrate-users/scim/scim-client-id-secret.png)
...
![A list including the default SCIM lambdas.](/img/docs/lifecycle/migrate-users/scim/scim-lambdas-list.png)

You may edit and modify lambdas as needed, but for the purposes of this document, the default lambdas work fine.

![The default SCIM user request lambda.](/img/docs/lifecycle/migrate-users/scim/scim-user-request-default-lambda.png)
...
To do so, navigate to <Breadcrumb>Tenants -> Default Tenant</Breadcrumb>. Edit the tenant and then navigate to the <Breadcrumb>SCIM</Breadcrumb> tab. Once you've checked the <Breadcrumb>Enabled</Breadcrumb> field, you'll see all the SCIM settings.

Configure the following details:

* The <InlineField>Client entity type</InlineField> should be the client entity type you previously created: <InlineFieldValue>SCIM Client</InlineFieldValue>.
* The <InlineField>Server entity type</InlineField> should be the client entity type you previously created: <InlineFieldValue>SCIM Server</InlineFieldValue>.
...
![A configured tenant](/img/docs/lifecycle/migrate-users/scim/tenant-scim-settings-after-configuration.png)
...
Then, run the command below, with the value of <InlineFieldValue>YOUR_CLIENT_SECRET</InlineFieldValue> updated to the real value, such as <InlineField>i2kN2KFBPKKj3FsxfilPXDDreBUskn2Sl-27QbQRYQM</InlineField>.
...
![New user added via SCIM.](/img/docs/lifecycle/migrate-users/scim/new-user-added.png)
...
* See [What is SCIM](/blog/what-is-scim) for a gentle introduction to SCIM.
* See the [SCIM Overview](/docs/lifecycle/migrate-users/scim) for details about SCIM.
