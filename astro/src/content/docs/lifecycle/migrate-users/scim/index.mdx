...
tags: scim, system for cross-domain identity management, identity management, cross-domain, provision user, user provision
---
...
```mermaid title="FusionAuth SCIM Workflow"
sequenceDiagram
  autonumber
  participant Client as SCIM Client
  participant FA as FusionAuth
  participant InLambda as Incoming Request Lambda
  participant OutLambda as Outgoing Response Lambda

  Client->>FA: SCIM Request (POST /api/scim/resource/v2/Users)
  FA->>FA: Authenticate Client (Client Credentials Grant)
  FA->>InLambda: Invoke with SCIM Request Data
  InLambda-->>FA: FusionAuth User Object
  FA->>FA: Create FusionAuth User
  FA->>OutLambda: Invoke with FusionAuth User & SCIM Response
  OutLambda-->>FA: SCIM Response Data
  FA-->>Client: SCIM Response (201 Created)
```
...
It is best practice for each tenant to have its own unique cryptographic key pair for signing tokens. Especially for SCIM, where you may have a SCIM configuration for each tenant likely from different sources, using a dedicated key pair limits the impact to a single tenant in case the cryptographic key pair is leaked or compromised.

To generate a new key pair in FusionAuth:
1. Navigate to <Breadcrumb>Settings -> Key Master</Breadcrumb>.
2. Select <Breadcrumb>Generate EC Key Pair</Breadcrumb> (or RSA if preferred).
3. Provide a name and choose the appropriate settings.
4. Once generated, you can assign this key to be used for JWT signing within your tenant's SCIM configuration.

![Generating an EC Key Pair in Key Master](/img/docs/lifecycle/migrate-users/scim/fusionauth-key-master-generate-ec-key-pair.png)

### Tenant Configuration

You need to enable SCIM support for your tenant. Navigate to <Breadcrumb>Tenants</Breadcrumb>.

![The tenant list in FusionAuth.](/img/docs/lifecycle/migrate-users/scim/fusionauth-tenant-list.png)

Edit the tenant and navigate to the <Breadcrumb>SCIM</Breadcrumb> tab. Check the <InlineField>Enabled</InlineField> field and configure the lambdas and entity types.

![The SCIM tab in the tenant configuration.](/img/docs/lifecycle/migrate-users/scim/fusionauth-tenant-scim-server-settings.png)

To assign the generated key pair for JWT signing, navigate to the <Breadcrumb>JWT</Breadcrumb> tab in the tenant configuration and select the key.

![The JWT tab in the tenant configuration.](/img/docs/lifecycle/migrate-users/scim/fusionauth-tenant-jwt-settings.png)
...
Navigate to <Breadcrumb>Entity Management -> Entities</Breadcrumb>.

![The entity list in FusionAuth.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-list.png)

#### SCIM Server Entity

Create a new Entity for the SCIM server using the `SCIM Server` entity type.

![Adding a SCIM server entity.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-add-scim-server.png)

#### SCIM Client Entity

Create a new Entity for the SCIM client using the `SCIM Client` entity type.

![Adding a SCIM client entity.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-add-scim-client.png)

#### Grants and Permissions

After creating both entities, you must grant the SCIM client permissions on the SCIM server. Navigate to the SCIM client entity and click the <InlineUIElement>Manage</InlineUIElement> button.

![Managing a SCIM client entity.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-manage-scim-client.png)

Click the <InlineUIElement>Add</InlineUIElement> button to add a grant for the SCIM server entity.

![Adding a grant to a SCIM client entity.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-grant-add-scim-client.png)

Select the SCIM server entity and the appropriate permissions.

![Editing permissions for a SCIM grant.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-grant-edit-scim-server-permissions.png)

After saving the grant, you can see it in the list of grants for the SCIM client.

![The list of grants for the SCIM client.](/img/docs/lifecycle/migrate-users/scim/fusionauth-entity-manage-scim-client-grants-list.png)
...
* Listen for the [user.create.complete](/docs/extend/events-and-webhooks/events/user/user-create-complete) webhook and add the registration using an API call on the receiver.
...
To authenticate with the FusionAuth SCIM server, you should use the OAuth2 [Client Credentials Grant](/docs/lifecycle/authenticate-users/oauth/#example-client-credentials-grant). This is the industry standard and recommended way to authenticate SCIM requests, as it aligns with the security practices outlined in [RFC 7644 Section 2](https://datatracker.ietf.org/doc/html/rfc7644#section-2) and [Section 7.4](https://datatracker.ietf.org/doc/html/rfc7644#section-7.4).

When using the Client Credentials Grant, the SCIM client dynamically requests an access token from FusionAuth using its client credentials. This token is then used in the `Authorization` header as a `Bearer` token for SCIM requests.

### Recommended Authentication Method

We strongly recommend using the [Client Credentials Grant](/docs/apis/authentication#client-credentials) over a static authorization header. This method is supported by major identity providers and offers several advantages:

* **Improved Security**: Tokens are short-lived (e.g., 60 minutes). If a token is compromised, its utility is limited.
* **Ease of Maintenance**: The SCIM client automatically renews the token. You don't need to manually update static keys when they expire.
* **Standardization**: It follows standard OAuth2 patterns used across the industry for machine-to-machine communication.

<Aside type="tip">
If your SCIM client does not offer a separate field for the `scope` parameter (such as Microsoft Entra ID), you can append it to the Token endpoint URL as a query string.

For example: `https://local.fusionauth.io/oauth2/token?scope=target-entity:{SCIM_SERVER_ENTITY_ID}:{PERMISSIONS}`
</Aside>

### Static Authorization Header (Discouraged)

Some legacy SCIM clients or specific environments may not support the dynamic Client Credentials grant. These clients require a static value to be manually entered into an "Authorization" or "Secret Token" field.

<Aside type="caution">
Using a static, long-lived token is discouraged due to several downsides:

* **Security Risk**: Because the token must be long-lived to avoid frequent manual updates, it essentially becomes a static API key. If this token is leaked, it provides persistent access to your SCIM endpoints.
* **Lack of Automatic Rotation**: You must manually regenerate and update the token in the SCIM client before it expires. If you forget, provisioning will fail.
* **Harder Revocation**: Revoking a long-lived JWT is more complex than simply updating the client secret and letting a short-lived token expire.
</Aside>

If your SCIM client does not support the [Client Credentials Grant](/docs/apis/authentication#client-credentials), you can manually generate a long-lived token by following these steps:

* Optionally change the JWT duration of the SCIM Client Entity Type to a larger value, such as a year, which is `31536000`. The generated JWT will not be able to be revoked, so pick a duration that balances the security risk with the effort of updating the JWT in the SCIM client configuration (Microsoft Entra ID, Okta, etc).
...
* Perform the [Client Credentials Grant](/docs/apis/authentication#client-credentials) manually using a tool like curl (see below for an example).
