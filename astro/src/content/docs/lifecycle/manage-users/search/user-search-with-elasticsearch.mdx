...
tags: search users, elasticsearch, opensearch, query users, read users
---
...
<Aside type="note">
While this document references Elasticsearch, the same functionality is available if you use a [supported version of OpenSearch](/docs/get-started/download-and-install/reference/system-requirements#elasticsearch).
</Aside>
...
![Determine the search engine.](/img/docs/lifecycle/manage-users/search/confirm-search-engine.png)
...
* Learn more about [each type of search engine](/docs/lifecycle/manage-users/search).
...
Let's look at some examples of `queryString` searches.

### All Fields

Below, you are searching for `dinesh`, across all fields, and wildcards are prepended and appended.
...
Below, only the <InlineField>email</InlineField> field is searched for the string <InlineFieldValue>dinesh</InlineFieldValue>, because <InlineField>email</InlineField>: is specified.
...
Finally, below, the <InlineField>email</InlineField> field matches the string <InlineFieldValue>dinesh</InlineFieldValue> and the <InlineField>verified</InlineField> field is `false`. These are joined together with an `OR` clause which returns the user if either clause is true, but you can also use `AND` for the intersection.
...
Below, only the `phoneNumber` field is searched for because `phoneNumber:` is specified. The query matches phone numbers with any country code and starting with the `303` area code.

<JSON title="Example Request JSON For Searching By Phone Number With queryString"
      src="users/search-queryString-phoneNumber-example-elastic-request.json" />
...
<Aside type="note">
The FusionAuth <InlineField>user.data</InlineField> field can hold arbitrary JSON and is useful if your users have customer data fields.
...
* For that particular application, having a <InlineField>paid</InlineField> attribute of `false`.

This is quite specific. In addition to matching a value in the <InlineField>user.data</InlineField> field like the first example, you are searching ranges, checking standard user attributes such as `verified`, finding users who have registered for a given application, and examining application specific fields.
