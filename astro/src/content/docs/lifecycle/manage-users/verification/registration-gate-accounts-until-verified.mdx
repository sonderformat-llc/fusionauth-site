...
<Aside type="note">
Version `1.59.0` deprecated registration verification. This feature will be removed in a future version of FusionAuth.
</Aside>
...
![Piper Web](/img/docs/lifecycle/manage-users/verification/application-registration-piedpiper.png)
...
<Aside type="note">
Email Verification and Application Registration Verification are not premium features, however, gating is. Gating stops the user from receiving a JWT or essentially authenticating any further. FusionAuth is enforcing a "gate" around a user until they verify their email/phone.
</Aside>
...
Navigate to <Breadcrumb>Tenants -> Your Tenant</Breadcrumb> and then select the <Breadcrumb>Email</Breadcrumb> tab. Scroll to the <Breadcrumb>SMTP settings</Breadcrumb> section. Configure this with your SMTP server information. If you are testing locally, you can use a mock email server like [mailcatcher](https://mailcatcher.me/).

![Tenant SMTP settings](/img/docs/lifecycle/manage-users/verification/tenant-smtp-settings.png)

Ensure a test email is delivered. If you have any issues, follow the steps in the [email troubleshooting documentation](/docs/operate/troubleshooting#troubleshooting-email).

Save the configuration.

## Enable Registration Verification Gating

Navigate to <Breadcrumb>Applications -> Your Application</Breadcrumb> and then select the <Breadcrumb>Registration</Breadcrumb> tab. Scroll to the <Breadcrumb>Registration verification settings</Breadcrumb> section.
...
- Update the <InlineField>Verification strategy</InlineField>. This can be either <InlineFieldValue>ClickableLink</InlineFieldValue> or <InlineFieldValue>FormField</InlineFieldValue>. The former sends an email with a link to verify the user's address and intent to register for your application. The latter emails a code which the user will provide to FusionAuth. For this tutorial, use <InlineFieldValue>ClickableLink</InlineFieldValue>.
- Ensure that the <InlineField>Unverified behavior</InlineField> value is <InlineFieldValue>Gated</InlineFieldValue>.

After these steps, you should end up with a configuration screen that looks like this:

![Application registration verification settings](/img/docs/lifecycle/manage-users/verification/application-registration-verification-settings.png)
...
![Enabling application self registration](/img/docs/lifecycle/manage-users/verification/application-enable-self-registration.png)

Make sure you save the configuration.

Next, navigate to the <Breadcrumb>OAuth</Breadcrumb> tab. Enable the <InlineFieldValue>Authorization Code</InlineFieldValue> grant.
...
Click on the <Breadcrumb>View</Breadcrumb> button of your application to see the details of your application.

![View application details to see the relevant URLs](/img/docs/lifecycle/manage-users/verification/view-application-details.png)

Look for the <InlineField>Registration URL</InlineField>. It'll be something like <InlineFieldValue>https://local.fusionauth.io/oauth2/register?client_id=85a03867-dccf-4882-adde-1a79aeec50df&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Foauth-redirect</InlineFieldValue>. Copy that value as you'll use it below.
...
Navigate to <Breadcrumb>Customizations -> Email Templates</Breadcrumb>. Edit the <InlineFieldValue>Registration Verification</InlineFieldValue> template by clicking on the blue <Breadcrumb>Edit</Breadcrumb> button.

Modify the HTML and text templates as desired and then save them. You can also localize the messages.

![Customizing the registration verification email template](/img/docs/lifecycle/manage-users/verification/registration-template-customization.png)
...
![Verification registration in mailcatcher](/img/docs/lifecycle/manage-users/verification/verification-registration-email-mailcatcher.png)
...
![Clickable link default registration verification gate screen](/img/docs/lifecycle/manage-users/verification/registration-verification-gate-screen-clickable-link.png)
...
If you view the user in the administrative user interface and click on the <Breadcrumb>source</Breadcrumb> tab you should see the following attributes on the user.

![Unverified user details screen](/img/docs/lifecycle/manage-users/verification/unverified-user-registration-details-screen.png)
...
<JSON title="Registrations Array Showing Verification Set To true/off JSON" src="registration-gate/user-registration-verification-turned-off-initially.json" />
...
<JSON title="Registrations Array Showing Verification Set To false/on JSON" src="registration-gate/user-registration-verification-turned-on-initially.json" />
...
![Code entry default registration gate screen](/img/docs/lifecycle/manage-users/verification/registration-verification-gate-screen-code-entry.png)
