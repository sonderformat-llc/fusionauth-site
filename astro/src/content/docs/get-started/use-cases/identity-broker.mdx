...
<Aside type="note">
  FusionAuth supports deployments in [air-gapped environments](/docs/get-started/download-and-install/reference/air-gapping).
</Aside>
...
You already have FusionAuth working for your application, but when you are building your deployment package, you'll need to include FusionAuth and a compatible database, as well as other [system requirements](/docs/get-started/download-and-install/reference/system-requirements).
...
Make sure you [configure FusionAuth correctly](/docs/reference/configuration). You'll usually want to deploy FusionAuth using the [database search engine](/docs/lifecycle/manage-users/search#using-the-database-search-engine) to make deployment simpler.
...
Give this user a known password that you can share with your customer. You can require them to change it at first login by setting the <InlineField>passwordChangeRequired</InlineField> field to <InlineFieldValue>true</InlineFieldValue> when creating the user.
...
* a known <InlineFieldValue>Id</InlineFieldValue> so that later <InlineField>idp_hint</InlineField> operations are simpler
* set `name` to `Customer identity provider`
* set <InlineField>debug</InlineField> to <InlineFieldValue>false</InlineFieldValue>
* set <InlineField>createRegistration</InlineField> to <InlineFieldValue>true</InlineFieldValue>
* set <InlineField>enabled</InlineField> to <InlineFieldValue>true</InlineFieldValue> for your application to enable this identity provider
...
* <InlineField>application.oauthConfiguration.requireRegistration</InlineField> to <InlineFieldValue>true</InlineFieldValue> to require user registration
...
* <InlineFieldValue>admin</InlineFieldValue>
* `developer`
* `business_analyst`
...
|--------------------|-----------------|------------------|
| <InlineFieldValue>admin</InlineFieldValue>            | <InlineField>administrator</InlineField> | <InlineField>superadmin</InlineField>     |
...
Here's the body of [an example OIDC reconcile lambda](/docs/extend/code/lambdas/reconcile/openid-connect-response-reconcile) which maps roles from a customer data store to the application roles:
...
```mermaid title="User logging in to the application."
sequenceDiagram
...
* [OIDC reconcile lambda](/docs/extend/code/lambdas/reconcile/openid-connect-response-reconcile)
* [SAMLv2 reconcile lambda](/docs/extend/code/lambdas/reconcile/samlv2-response-reconcile)
