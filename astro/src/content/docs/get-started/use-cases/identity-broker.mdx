...
Give this user a known password that you can share with your customer. You can require them to change it at first login by setting the <InlineField>passwordChangeRequired</InlineField> field to `true` when creating the user.
...
* a known <InlineFieldValue>Id</InlineFieldValue> so that later `idp_hint` operations are simpler
...
|--------------------|-----------------|------------------|
| <InlineFieldValue>admin</InlineFieldValue>            | `administrator` | `superadmin`     |
