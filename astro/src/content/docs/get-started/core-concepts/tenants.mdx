...
import TemplateSettings from 'src/content/docs/get-started/core-concepts/_template-settings.mdx';
...
![List of Tenants](/img/docs/get-started/core-concepts/tenant-configuration-list.png)
...
### General
![Tenant Configuration - General](/img/docs/get-started/core-concepts/tenant-configuration-general.png)
...
![The Tenant Connector policy configuration tab.](/img/docs/get-started/core-concepts/tenant-connector-tab.png)
...
If you click on the <Breadcrumb>Add policy</Breadcrumb> button on this page you will be presented with the following dialog.

![Add Connector Policy](/img/docs/get-started/core-concepts/tenant-configuration-connector-add-policy.png)
...
![Tenant Configuration - Identities settings](/img/docs/get-started/core-concepts/tenant-configuration-identities-settings.png)

#### Messaging Settings

<APIBlock>
  <APIField name="Messenger" required since="1.59.0">
    ...
    Required when the <InlineField>Verify identity</InlineField> toggle is enabled for phone numbers or when any phone templates are configured in <Breadcrumb>Template Settings</Breadcrumb>.
  </APIField>
</APIBlock>
...
<APIBlock>
  <APIField name="Verify identity" optional>
  ...
  <APIField name="Verify identity when changed" optional>
  ...
  <APIField name="Verification template" required>
    ...
    Required when the associated <InlineField>Verify identity</InlineField> toggle is enabled.
  </APIField>
  ...
  <APIField name="Verification strategy" optional since="1.27.0">
    The process by which the user will verify their email address or phone number. Using the `Form Field` method works only when the associated <InlineField>Unverified behavior</InlineField> is `Gated`.
  </APIField>
  ...
  <APIField name="Allow change when gated" optional since="1.27.0">
...
![Tenant Configuration - Template settings](/img/docs/get-started/core-concepts/tenant-configuration-template-settings.png)
...
Once you have configured your email settings, you may test your configuration with the <Breadcrumb>Send test email</Breadcrumb> button.

![Tenant Configuration - SMTP settings](/img/docs/get-started/core-concepts/tenant-configuration-smtp-settings.png)
...
<APIBlock>
  ...
  <APIField name="Security" optional defaults="None">
    ...
    Generally speaking, if using port `25` you will select <InlineFieldValue>None</InlineFieldValue>, if using port of `465` you will select <InlineFieldValue>SSL</InlineFieldValue> and if using port `587` you will select <InlineFieldValue>TLS</InlineFieldValue>. It is possible your provider will be different, follow your providers instruction.
...
### Family
![Tenant Configuration - Family](/img/docs/get-started/core-concepts/tenant-configuration-family-settings.png)
...
<APIBlock>
  ...
  <APIField name="Maximum child age" required>
    ...
    Required when the <Breadcrumb>Enabled</Breadcrumb> toggle is enabled.
  </APIField>
...
![Tenant Configuration - MFA policy settings](/img/docs/get-started/core-concepts/tenant-configuration-mfa-policy-settings.png)
...
<APIBlock>
  ...
  <APIField name="MFA requirement lambda" optional>
    ...
    * Trigger a suspicious login event based on your criteria. &nbsp; <AvailableSince since="1.62.0"/>
    <EnterprisePlanBlurb />
  </APIField>
</APIBlock>

![Tenant Configuration - MFA authenticator settings](/img/docs/get-started/core-concepts/tenant-configuration-mfa-authenticator-settings.png)
...
![Tenant Configuration - MFA email settings](/img/docs/get-started/core-concepts/tenant-configuration-mfa-email-settings.png)
...
![Tenant Configuration - MFA SMS settings](/img/docs/get-started/core-concepts/tenant-configuration-mfa-sms-settings.png)
...
![Tenant Configuration - WebAuthn](/img/docs/get-started/core-concepts/tenant-configuration-webauthn.png)
...
![Tenant Configuration - WebAuthn Bootstrap settings](/img/docs/get-started/core-concepts/tenant-configuration-webauthn-bootstrap-settings.png)
...
![Tenant Configuration - WebAuthn Re-authentication settings](/img/docs/get-started/core-concepts/tenant-configuration-webauthn-reauthentication-settings.png)
...
### OAuth
![Tenant Configuration - OAuth](/img/docs/get-started/core-concepts/tenant-configuration-oauth.png)
...
<APIBlock>
  ...
  <APIField name="Allow access token bootstrap" since="1.56.0">
...
### Password
![Tenant Configuration - JWT](/img/docs/get-started/core-concepts/tenant-configuration-password-failed-authentication-settings.png)
...
![Tenant Configuration - JWT](/img/docs/get-started/core-concepts/tenant-configuration-password-breach-detection-settings.png)
...
![Tenant Configuration - JWT](/img/docs/get-started/core-concepts/tenant-configuration-password-validation-settings.png)

#### Password settings

<APIBlock>
  <APIField name="Password enabled" optional defaults="enabled" since="1.59.0">
  ...
  <APIField name="Reject passwords containing user login Id" optional since="1.63.0">
...
![Tenant Configuration - JWT](/img/docs/get-started/core-concepts/tenant-configuration-password-cryptographic-hash-settings.png)
...
![Tenant Configuration - SCIM](/img/docs/get-started/core-concepts/tenant-configuration-scim-settings.png)

Read more about setting up and using SCIM in the [full SCIM documentation](/docs/lifecycle/migrate-users/scim).
...
![Tenant Configuration - Security: ACL and CAPTCHA](/img/docs/get-started/core-concepts/tenant-configuration-security-acl-catpcha.png)
...
CAPTCHA is supported on multiple theme templates, when enabled. You can further control display on a template by template basis with the [specific theme template variables](/docs/customize/look-and-feel/advanced-themes/template-variables).

See [invisible reCAPTCHA](/docs/customize/look-and-feel/advanced-themes/helpers#invisible-recaptcha) for information on enabling an invisible reCAPTCHA badge on the page.
...
![Tenant Configuration - Security: Blocked domains and rate limiting](/img/docs/get-started/core-concepts/tenant-configuration-security-domains-rate-limiting.png)
...
![Tenant Configuration - External Identifier Durations](/img/docs/get-started/core-concepts/tenant-configuration-extId-durations.png)
...
<APIBlock>
  ...
  <APIField name="Phone verification" required>
...
![Tenant Configuration - External Identifier Generation](/img/docs/get-started/core-concepts/tenant-configuration-extId-generation.png)
...
<APIBlock>
  ...
  <APIField name="Passwordless Login one-time code" required since="1.59.0">
    The length and type of characters used for the generated codes in the Passwordless Login flow.

    The one-time code for passwordless logins is only used when sending SMS messages to phone numbers, or when using an explicit `loginStrategy` value of `FormField` on the [Start Passwordless Login API](/docs/apis/passwordless#start-passwordless-login).
  </APIField>
  <APIField name="Phone verification" required>
    The length and type of characters of the generated code used in the Phone Verification flow.
  </APIField>
  <APIField name="Phone verification one-time code" required>
