...
Before continuing with this guide, please go through the [FusionAuth guide to monitoring](/docs/operate/monitor) to get an overview of the available metrics and choose the ones relevant to your needs.
...
services:
  ...
  fa:
    ...
    environment:
      ...
      FUSIONAUTH_APP_INSTALLATION_SOURCE: site/monitor-splunk
...
On the Splunk website, go to the <InlineUIElement>Metric Finder</InlineUIElement> section and search for `otel` to see if any data is visible. Even when FusionAuth is not running, the collector will send basic data to Splunk.
...
FusionAuth has too [many metrics](/docs/operate/monitor#metrics) to discuss in this article. You will need to decide which are important for you to monitor by reading the documentation.
...
The second half of the script reads in the CSV file, discards the header, and sends a value of <InlineFieldValue>1</InlineFieldValue> and the time of each login to Splunk.
...
- [FusionAuth metrics](/docs/operate/monitor#metrics)
