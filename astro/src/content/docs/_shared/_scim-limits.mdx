...
You cannot create a FusionAuth Registration for a user provisioned with SCIM directly during the provisioning process. See [Adding Registrations](/docs/lifecycle/migrate-users/scim#adding-registrations) for workarounds.
...
Okay, okay, you want multiple SCIM clients to provision users into a single FusionAuth tenant. Let's talk about the complexities. Imagine you have an Okta SCIM client provisioning users into a FusionAuth tenant and a Microsoft Entra ID SCIM client provisioning users into the same tenant.

Issues with this approach include:

* The user and group lambdas would have to include logic to differentiate between a user from Okta and a user from Microsoft Entra ID and handle the fields differently.
* User collisions are not handled by FusionAuth. If the Okta SCIM client created an account for a user with the email address `richard@example.com` in the tenant, then later the Microsoft Entra ID SCIM client tried to create an account for a user with the email address `richard@example.com`, the second account creation would fail.
* Update or deletion requests from the Okta SCIM client could modify users created by the Microsoft Entra ID SCIM client and vice versa.
* The bearer access token used to authenticate each SCIM client can be different, but they must have the same lifetime, since that is configured at the Entity Type level. You couldn't have Okta's credentials be valid for a year but Microsoft Entra ID's credentials be valid for six months.
