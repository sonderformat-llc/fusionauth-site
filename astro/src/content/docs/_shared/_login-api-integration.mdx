...
If you require a refresh token after completing the login, ensure <InlineField>Enable JWT Refresh</InlineField> is `<InlineFieldValue>true</InlineFieldValue>` in the application configuration. This is found in the administrative user interface by navigating to <Breadcrumb>Applications -> Your Application -> Security -> Login API settings</Breadcrumb>.
