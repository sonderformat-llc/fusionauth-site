---
title: Login API
description: Explore the FusionAuth Login API documentation for detailed information on authenticating users, handling sessions, and implementing secure login flows.
---
...
<LoginMetadataDevice />
  ...
  <APIField name="noJWT" type="Boolean" optional defaults="false">
    When this value is set to `true` a JWT will not be issued as part of this request. The response body will not contain the `token` or <InlineField>refreshToken</InlineField> fields, and the `access_token` and `refresh_token` cookies will not be written to the HTTP response.
...
<LoginMetadataDevice />
  ...
  <APIField name="noJWT" type="Boolean" optional defaults="false">
    When this value is set to `true` a JWT will not be issued as part of this request. The response body will not contain the <InlineField>token</InlineField> field,
...
<APIBlock>
  ...
  <APIField name="logins[x].loginId" type="String">
    ...
    In version `1.59.0` and later, the <InlineField>loginId</InlineField> is stored in the login record when the correct value can be identified. In these cases the <InlineField>loginIdType</InlineField> will be populated.
  </APIField>
