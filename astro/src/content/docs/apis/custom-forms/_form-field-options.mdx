...
The options for a field must conform to the <InlineField>field.type</InlineField>. If the <InlineField>field.type</InlineField> is <InlineFieldValue>number</InlineFieldValue>, the options must be numbers. If the <InlineField>field.type</InlineField> is <InlineFieldValue>boolean</InlineFieldValue>, only `true` and/or `false` are allowed.
