...
You must provide either the <InlineField>email</InlineField> or the <InlineField>username</InlineField> field for each User. This will be used to authenticate the User and is also the User's unique identifier. These fields are marked as optional below, but you must provide one of them. The <InlineField>password</InlineField> field (plaintext or hash) is also required.
...
<APIField name="users[x].mobilePhone" type="String" optional>
  ...
  <Aside>
    The phone number in this field can't be used for login. Use <InlineField>users\[x].phoneNumber</InlineField> for a phone that can be used for login.
  </Aside>
</APIField>

<APIField name="users[x].password" type="String" optional>
  ...
  <Aside title="Since 1.59.0" type="version">
...
<APIField name="users[x].phoneNumber" type="String" optional since="1.59.0">
  The phone number of the User.

  <Aside>
    Use this field instead of <InlineField>users\[x].mobilePhone</InlineField> for a phone number that is used to log in, just like an email address or a username.
  </Aside>
</APIField>
...
<APIField name="users[x].preferredLanguages" type="Array<String>" optional>
  ...
  Use the language preferences for email templates and other localized text. For more information, see [Locales](/docs/reference/data-types#locales).
</APIField>
...
<APIField name="users[x].twoFactor.methods[x].secret" type="String" optional>
  ...
  This field is required when <InlineField>method</InlineField> is <InlineFieldValue>authenticator</InlineFieldValue>.
</APIField>
...
<APIField name="users[x].twoFactorSecret" type="String" optional deprecated>
  ...
  When using <InlineFieldValue>None</InlineFieldValue> as the `twoFactorDelivery` this value will be required.
...
<APIField name="users[x].verifiedInstant" type="Long" optional defaults="now">
  The [instant](/docs/reference/data-types#instants) this user's email address or phone number was verified. This value will be ignored if <InlineField>verified</InlineField> is `false`.
</APIField>
